...
{
  DWORD proj = 0;
       if (SendDlgItemMessage(hwnd, IDC_PROJ_MERCATOR, BM_GETCHECK, 0, 0) == BST_CHECKED) proj = 1;
  else if (SendDlgItemMessage(hwnd, IDC_PROJ_CYLINDRICAL, BM_GETCHECK, 0, 0) == BST_CHECKED) proj = 2;
  float rot = GetDlgItemFloat(hwnd, IDC_ROT, 0);
  BOOL rot_galactic = SendDlgItemMessage(hwnd, IDC_ROT_GALACTIC, BM_GETCHECK, 0, 0) == BST_CHECKED;
  ...
  if (save) {
    Settings.proj = proj;
    Settings.rot = rot;
    Settings.rot_galactic = rot_galactic;
    Settings.mag = mag;
...
BOOL CALLBACK PropertiesImageProc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
  switch (msg) {
    case WM_COMMAND:
      switch (HIWORD(wparam)) {
        case BN_CLICKED:
          ...
          } else if (LOWORD(wparam) == IDC_ROT_GALACTIC) {
            BOOL enable = SendDlgItemMessage(hwnd, IDC_ROT_GALACTIC, BM_GETCHECK, 0, 0) != BST_CHECKED;
            EnableWindow(GetDlgItem(hwnd, IDC_ROT), enable);
          }
          // FALL THROUGH //
        case EN_CHANGE:
    ...
    case WM_INITDIALOG: {
      ...
      SendDlgItemMessage(hwnd, IDC_PROJ_CYLINDRICAL, BM_SETCHECK, Settings.proj == 2, 0);
      SetDlgItemFloat(hwnd, IDC_ROT, Settings.rot);
      SendDlgItemMessage(hwnd, IDC_ROT_GALACTIC, BM_SETCHECK, Settings.rot_galactic, 0);
      ...
      if (Settings.rot_galactic) {
        EnableWindow(GetDlgItem(hwnd, IDC_ROT), FALSE);
      }
      break;
    }
    case WM_NOTIFY:
...
bool GetSettingsViewpoint(HWND hwnd, bool save)
{
  ...
  else if (SendDlgItemMessage(hwnd, IDC_POS_MOON   , BM_GETCHECK, 0, 0) == BST_CHECKED) pos.type = Position::moon;
...
BOOL CALLBACK PropertiesViewpointProc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
  switch (msg) {
    ...
    case WM_INITDIALOG: {
      ...
      SendDlgItemMessage(hwnd, IDC_POS_MOON, BM_SETCHECK, Settings.pos.type == Position::moon, 0);
...
void FillQuakeList(HWND list)
{
  ListView_DeleteAllItems(list);
  std::list<quake> quakes = GetQuakes();
  for (std::list<quake>::const_iterator q = quakes.begin(); q != quakes.end(); q++) {
    char buf[80];
    GetDateFormat(GetThreadLocale(), 0, &q->time, "yyyy-MM-dd", buf, sizeof(buf));
    strcat(buf, " ");
    GetTimeFormat(GetThreadLocale(), 0, &q->time, "HH:mm", &buf[strlen(buf)], sizeof(buf)-strlen(buf));
    LV_ITEM lvi;
    lvi.mask = LVIF_TEXT | LVIF_PARAM;
    lvi.iItem = 0;
    lvi.iSubItem = 0;
    lvi.pszText = buf;
    lvi.lParam = reinterpret_cast<LPARAM>(new quake(*q));
    int index = ListView_InsertItem(list, &lvi);
    sprintf(buf, "%.2f", q->lat);
    ListView_SetItemText(list, index, 1, buf);
    sprintf(buf, "%.2f", q->lon);
    ListView_SetItemText(list, index, 2, buf);
    sprintf(buf, "%.1f", q->mag);
    ListView_SetItemText(list, index, 3, buf);
    ListView_SetItemText(list, index, 4, const_cast<char *>(q->location.c_str()));
  }
}

int CALLBACK CompareQuakeTime(LPARAM p1, LPARAM p2, LPARAM p)
{
  quake *q1 = reinterpret_cast<quake *>(p1);
  quake *q2 = reinterpret_cast<quake *>(p2);
  unsigned __int64 t1, t2;
  SystemTimeToFileTime(&q1->time, reinterpret_cast<FILETIME *>(&t1));
  SystemTimeToFileTime(&q2->time, reinterpret_cast<FILETIME *>(&t2));
  if (t1 > t2) {
    return -1;
  } else if (t1 < t2) {
    return 1;
...
int CALLBACK CompareQuakeLatitude(LPARAM p1, LPARAM p2, LPARAM p)
{
  quake *q1 = reinterpret_cast<quake *>(p1);
  quake *q2 = reinterpret_cast<quake *>(p2);
  if (q1->lat < q2->lat) {
    return -1;
  } else if (q1->lat > q2->lat) {
    return 1;
  }
  return 0;
}

int CALLBACK CompareQuakeLongitude(LPARAM p1, LPARAM p2, LPARAM p)
{
  quake *q1 = reinterpret_cast<quake *>(p1);
  quake *q2 = reinterpret_cast<quake *>(p2);
  if (q1->lon < q2->lon) {
    return -1;
  } else if (q1->lon > q2->lon) {
    return 1;
  }
  return 0;
}

int CALLBACK CompareQuakeMagnitude(LPARAM p1, LPARAM p2, LPARAM p)
{
  quake *q1 = reinterpret_cast<quake *>(p1);
  quake *q2 = reinterpret_cast<quake *>(p2);
  if (q1->mag > q2->mag) {
...
int CALLBACK CompareQuakeLocation(LPARAM p1, LPARAM p2, LPARAM p)
{
  quake *q1 = reinterpret_cast<quake *>(p1);
  quake *q2 = reinterpret_cast<quake *>(p2);
  return q1->location.compare(q2->location);
}

bool GetSettingsQuakes(HWND hwnd, bool save)
{
  BOOL quakes = SendDlgItemMessage(hwnd, IDC_QUAKES, BM_GETCHECK, 0, 0) == BST_CHECKED;
  DWORD qdelay = GetDlgItemInt(hwnd, IDC_QDELAY, NULL, FALSE);
  if (save) {
    Settings.quakes = quakes;
    Settings.qdelay = qdelay;
  }
  return true;
}

BOOL CALLBACK PropertiesQuakesProc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
  switch (msg) {
    case WM_COMMAND:
      switch (HIWORD(wparam)) {
        case BN_CLICKED:
          if (LOWORD(wparam) == IDC_QUAKES) {
            BOOL enable = SendDlgItemMessage(hwnd, IDC_QUAKES, BM_GETCHECK, 0, 0) == BST_CHECKED;
            EnableWindow(GetDlgItem(hwnd, IDC_QDELAY), enable);
          } else if (LOWORD(wparam) == IDC_QUPDATE) {
            UpdateQuakes();
            break;
          }
          // FALL THROUGH //
      ...
      break;
    case WM_DESTROY: {
      HWND list = GetDlgItem(hwnd, IDC_QUAKELIST);
      for (int i = 0; i < ListView_GetItemCount(list); i++) {
        LV_ITEM lvi;
        lvi.mask = LVIF_PARAM;
        lvi.iItem = i;
        ListView_GetItem(list, &lvi);
        delete reinterpret_cast<quake *>(lvi.lParam);
      }
      KillTimer(hwnd, 1);
      break;
    }
    case WM_INITDIALOG: {
      ...
      SendDlgItemMessage(hwnd, IDC_QUAKES, BM_SETCHECK, Settings.quakes, 0);
      SetDlgItemInt(hwnd, IDC_QDELAY, Settings.qdelay, FALSE);
      HWND list = GetDlgItem(hwnd, IDC_QUAKELIST);
      LV_COLUMN lvc;
      lvc.mask = LVCF_FMT | LVCF_TEXT | LVCF_WIDTH;
      lvc.fmt = LVCFMT_LEFT;
      lvc.cx = 90;
      lvc.pszText = "Time (UTC)";
      ...
      if (Settings.qupdated) {
        ...
        st.wMilliseconds = 0;
        char buf[80];
        GetDateTimeFormat(GetThreadLocale(), 0, TIME_NOSECONDS, &st, buf, sizeof(buf));
        SetDlgItemText(hwnd, IDC_QUPDATED, buf);
      } else {
        SetDlgItemText(hwnd, IDC_QUPDATED, "Never");
      }
      FillQuakeList(list);
      if (!Settings.quakes) {
        EnableWindow(GetDlgItem(hwnd, IDC_QDELAY), FALSE);
      }
      break;
    }
    case WM_NOTIFY:
      switch (((NMHDR *)lparam)->code) {
        case LVN_COLUMNCLICK: {
          NM_LISTVIEW *nlv = reinterpret_cast<NM_LISTVIEW *>(lparam);
          switch (nlv->iSubItem) {
            case 0: ListView_SortItems(GetDlgItem(hwnd, IDC_QUAKELIST), CompareQuakeTime, 0); break;
            case 1: ListView_SortItems(GetDlgItem(hwnd, IDC_QUAKELIST), CompareQuakeLatitude, 0); break;
            case 2: ListView_SortItems(GetDlgItem(hwnd, IDC_QUAKELIST), CompareQuakeLongitude, 0); break;
            case 3: ListView_SortItems(GetDlgItem(hwnd, IDC_QUAKELIST), CompareQuakeMagnitude, 0); break;
            case 4: ListView_SortItems(GetDlgItem(hwnd, IDC_QUAKELIST), CompareQuakeLocation, 0); break;
          }
          break;
        }
        case PSN_APPLY: {
          GetSettingsQuakes(hwnd, true);
          DoApply(hwnd);
          break;
        }
        case PSN_KILLACTIVE: {
          if (!GetSettingsQuakes(hwnd, false)) {
            SetWindowLong(hwnd, DWL_MSGRESULT, TRUE);
          }
          break;
        }
    ...
    case WM_TIMER:
      if (QuakesUpdated()) {
        ...
        GetDateTimeFormat(GetThreadLocale(), 0, TIME_NOSECONDS, &st, buf, sizeof(buf));
        SetDlgItemText(hwnd, IDC_QUPDATED, buf);
        FillQuakeList(GetDlgItem(hwnd, IDC_QUAKELIST));
      }
      break;
    default:
      return FALSE;
